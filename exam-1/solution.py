...
def remove_out_of_range_entries(data, lat_range, lng_range):
    ...
    ## place your code here to complete this method according to the instructions above
    for line in data:
        if float(line["latitude"]) < lat_range[0] or float(line["latitude"]) > lat_range[1] or float(line["longitude"]) < lng_range[0] or float(line["longitude"]) > lng_range[1]:
            data.remove(line)
    return data
...
def make_location_title_case(data):
    ...
    ## place your code here to complete this method according to the instructions above
    for line in data:
        line["location"] = line["location"].title()
    return data
...
def fix_provider(data, old_provider, new_provider):
    ...
    ## place your code here to complete this method according to the instructions above
    for line in data:
        if line["provider"] == old_provider:
            line["provider"] = new_provider
    return data
...
def save_csv_data(data, filepath):
    ...
    csv_writer = csv.DictWriter(f, fieldnames=keylist)
...
def get_number_free_hotspots(filepath, neighborhood):
    ...
    for line in csv_reader:
        if line["type"] == "Free" and line["nta"] == neighborhood:
            count += 1
    return count
